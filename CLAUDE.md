...
| 子包 | 放什么 | 可以 import |
|---|---|---|
| `cli/` | 一个子命令一个模块（`task` `run` `loop` `cap` `flow` `status`），`__init__` 装配 parser 并导出 `main`；`cap` 的子命令从能力描述符生成（位置参数按 level：run 级 run_id、task 级任务包目录），`flow check` 按描述符对吃吐文件说一串能力通不通 | 下面全部 |
