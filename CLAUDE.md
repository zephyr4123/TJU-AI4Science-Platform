...
| `domains/` | 领域包，按工具链命名，一个领域一个目录；`generic/` 兜底、`petab/` 参数估计；`prompts/<能力>.md` 与 `skills/*/SKILL.md` 由 `cap auto-research` 开 run 时快照进 run、随执行层提示的「领域约定」段注入（执行层的隔离参数关掉了 CLI 原生 skill 加载）。纲领 P-18 里领域包是「包裹」：里面的 skill 拆开各归各的能力 |
...
| `workflows/` | 工作流**库**：通用的走法，不依附课题，一个一个 YAML（`name` / `title` / `summary` / `rooms`；一项是房间名、`房间: [能力]`、`房间: {能力: 参数}`、`断点` 或 `断点: 一句话`，「发布」「验收」是出厂的两个断点；纲领 P-18）。编辑台的造流助理与 `POST /workflows` 改它；主页面的研究助理只读，`ai4sci flow take <name>` 复制成工作区 `flows/` 里的实例再改参数、开 run 时快照进 run（流分三层：库 → 实例 → 快照）。`ai4sci show workflows` / `show flows` / `GET /workflows` / `GET /workspaces/<id>/flows` 读它们，只查房间名、点名的能力在不在那一间、参数、断点位置（房间之间不接管子）；`covers` `remarks` `used_by` 都是算出来的，文件里不写。出厂只有一条 `research` |
...
| 子包 | 放什么 | 可以 import |
|---|---|---|
| `cli/` | 命令行上就五类东西，一类一个模块：`cap` 能力（agent 调用的 tool；子命令从描述符生成：task 级不带位置参数、动的是当前工作区的任务包，run 级带 `run_id`，`--backend` / `--compute` 按需要，每个 Param 一个选项，bool 是开关；每颗都有 `--detach`：同一条命令起成独立进程当作业，子进程跑完回写记录、属于某段对话的去叫醒；run 级能力跑成后记它走到流的哪一间）、`sign` 人的确认（`task` 发布当前工作区的需求、`run` 验收）、`show` 查询（`workspaces` / `task` / `run`（含作业与便条）/ `jobs` / `job` / `flows`（工作区的流实例）/ `caps` 七间房各有什么、每颗五栏 / `workflows`（库），只读，与 serve 的 GET 同一批函数）、`flow take` 取流（库里的一条复制成工作区的实例）、`workspace new` / `chat`（`--studio` 选编辑台的造流助理）/ `serve` 入口；`_common.current_workspace` 从 cwd 往上找 `workspace.yaml`（`AI4SCI_WORKSPACE` 可指定），找不到退 2 说清怎么办；`__init__` 装配 parser 并导出 `main` | 下面全部 |
| `capabilities/` | 一个能力一个子包，互不 import，每个导出 `DESCRIPTOR`（属于哪一间 + 五栏：干什么 / 不干什么 / 要带什么进来 / 留下什么 / 什么时候停，讲机制、带专业术语；纲领 P-18）与 `run(run_dir, ports, **params)`（task 级是 `run(workspace, ports, ...)`，动的是工作区的任务包），`discover()` 扫目录并按 level 断言签名；子包名下划线对命令名连字符。能力之间不接管子：没有吃吐路径表，要的东西不在自己进门报错。task 级：`init/` 说清课题（假设间：建工作区的 `task/`、搬材料、放模板）、`design/` 写裁判、跑基线（设计间：前半段在 executor，后半段 `baseline.py`，日志落工作区 `runs/design/`）、`auto_research/` auto-research（实验间：run 不在就 `run/lifecycle.new_run` 建一个、`--workflow` 只认工作区 `flows/` 里的实例，然后内环 `loop` / `judge` / `gate` / `failures` / `prompt.md`）；run 级：`analysis/` 写分析初稿（`analyze` + `prompt.md`）、`verify/` 核对数字（`checks` 零模型） | executor、memory、run、contracts |
| `chat/` | 两位助理的对话与页面后端：`scope` 定域（工作区：cwd 是工作区、可写 `task/` `flows/` `runs/`、库 `workflows/` 可读不可写（端口 `readable_paths`，适配器 `--add-dir`）、对话在工作区 `chats/`；编辑台：cwd 是库的上级、只可写 `workflows/`、对话在 `studio/chats/`），`guide` 按域把 `coordinator/README.md` 或 `studio.md` 加前言塞进 system prompt（Bash 放行 `ai4sci`，带路径的老写法也放行，指南只教裸写法，红线 10 11）、`conversation` 建对话 / 发一轮 / 落盘 `<域>/chats/<id>/`（meta、每轮 message 与原生事件流、transcript、忙锁、`read_turns` / `title` 读回结构；一轮有 `origin`：人，或框架来叫醒）、`notify` 作业跑完以「框架」身份给同一个工作区里的那段对话发一轮（忙就等，等不到记回作业）、`boards` 看板读盘（工作区一行与一整份、任务包的阶段与钥匙、预检；run 的 best、账本、分析、验证、验收；NaN 出门前换 None）、`server` 标准库 HTTP + SSE（端点清单在文件头，按 `/workspaces/<id>/…` 与 `/studio/…` 分域；`/stages` 直接读契约，`/cap` `/workflows` `/workflows/check` 与工作区的流实例由 cli 以函数传入；不是接口前缀的 GET 路径端 `ui_dir` 的静态文件，单页应用回 index.html） | memory、run、contracts |
...
| `run/` | 一个 run 的磁盘状态与它的家：`workspace` 工作区（标记、六个目录、从 cwd 往上找、起与列）、`layout` 路径、`checkpoint`、`context` 只读上下文、`lifecycle` 建 run / 续命 / 能力目录轮转、`gitwork`、`artifacts` 结果索引、`accept` 验收记录、`jobs` 后台作业（只认作业目录 `<工作区>/jobs/`：记录 + 日志，独立会话起 `framework.cli`，pid 探活判 lost）、`flow_state` 便条（`cap auto-research --workflow` 开 run 时快照那条流进 `runs/<id>/workflow/`，`flow.json` 记走到第几项——项是房间或断点，点名看名字、不点名看阶段——在等谁现算）（`accept.json` 签 best 与验证结论，best 变了记录就 stale；与 `contracts.publish` 对称，放这层因为它读 checkpoint） | contracts |
| `contracts/` | `schemas/*.json`、任务包发现与校验 `packs`、发布记录 `publish`（钥匙）、接任务预检 `headroom`（门高的唯一定义）、工作流文件 `workflows`（房间 + 断点的解析与检查）、任务环境 `env`（读 env/、uv 建 venv）、产物读取 `results`、能力描述符与入口形状 `capability`（`STAGES` 七间房：每颗能力 `stage` 归一间，`title` 与五栏 `COLUMNS` 是给人读的，界面不另抄）、`analysis.md` 数据表契约 `analysis`、验证报告 `report` | 谁都不 import（framework 内） |
