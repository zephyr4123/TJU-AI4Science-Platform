...
| `workflows/` | 工作流**库**：通用的走法，不依附课题，一个一个 YAML（`name` / `title` / `summary` / `rooms`；一项是阶段名、`阶段: [能力]`、`阶段: {能力: 参数}`、`断点` 或 `断点: 一句话`，「发布」「验收」是出厂的两个断点；纲领 P-18）。编辑台的造流助理与 `POST /workflows` 改它；主页面的研究助理只读，`ai4sci flow take <name>` 复制成工作区 `flows/` 里的实例再改参数、开 run 时快照进 run（流分三层：库 → 实例 → 快照）。`ai4sci show workflows` / `show flows` / `GET /workflows` / `GET /workspaces/<id>/flows` 读它们，只查阶段名、点名的能力在不在那个阶段、参数、断点位置（阶段之间没有显式的输入输出接口、不做数据流校验）；`covers` `remarks` `used_by` 都是算出来的，文件里不写。出厂只有一条 `research` |
