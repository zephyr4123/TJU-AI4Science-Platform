...
- **编辑台**（改库，[#100](https://github.com/zephyr4123/TJU-AI4Science/issues/100)）：左边造流助理的对话（窄一列，可收起），右边一张画布（React Flow，点阵底）。顶上一条：name、标题、说明三个输入框、覆盖同名、保存；画布顶上一条：七个阶段的片（宋体、靛色图标）、三种断点的片（琥珀）、「库 N」弹层；片拖进画布落在哪条边上就插在哪，点一下加到末尾。节点是一张卡：阶段卡（SpotlightCard 改装：图标、宋体阶段名、序号、点名的能力做小片，没点名写「不限能力」）、断点卡（琥珀底：「断点」+ 确认事项，发布盖章、验收签名）；边是折线带箭头，只表示顺序；一行放不下换行，坐标不进文件。选中一个节点右边浮出配置：阶段 → 这个阶段的能力逐个勾选、勾上就按描述符给参数输入框（布尔是 SquishSwitch，数字 / 字符串是输入框，空着就是缺省值；每次调用时才定的参数如 run_id 不给）、箭头展开五栏；断点 → 「发布」「验收」两个快捷、一个确认事项输入框。检查结果在左下角：通过就说经过哪几个阶段，有问题一条一条列、并在节点上打红点（悬停看句子）；提醒琥珀色。窄屏配置改成右侧抽屉。
