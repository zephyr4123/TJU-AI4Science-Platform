...
给不写代码的研究者用的科研工作台。页面头号任务一句话：现在做到哪一步、在等谁、数字可不可信。世界观来自实验记录本和裁判——账本、基线、统计门、验证、发布与验收两处人的确认。视觉从这儿长，不从 SaaS 模板长。
