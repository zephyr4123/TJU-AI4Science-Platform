...
- **编辑台**（改库，[#100](https://github.com/zephyr4123/TJU-AI4Science/issues/100)）：左边造流助理的对话（窄一列，可收起），右边一整张画布（React Flow），底下压一层风景（晨光山脊线，mist 纱幕，与脊柱那列同一做法），点阵铺在风景上。不是表单：左上角一根**阶段梯**——七个研究阶段按顺序竖排成方块（大图标、宋体名），底下三种断点（琥珀）；梯子右边是**题头**，像图纸的标题栏——宋体大标题、mono 名字、一行说明，三条填空线不是输入框；右上角「库 N」弹层与圆角「保存」，名字与库里重了才出现「覆盖同名」的开关。梯子上的块拖进画布落在哪条边上就插在哪，点一下加到末尾。节点是一张卡：阶段卡（SpotlightCard 改装：22px 图标、宋体阶段名、点名的能力做靛色小片，没点名写「不限能力」，右下角一枚淡的宋体大序号——序号是文件里真实的第几项）、断点卡（琥珀底：图标 + 确认事项两行，发布盖章、验收签名）；边是折线带箭头，只表示顺序；一行放不下换行，坐标不进文件。选中一个节点右上角浮出配置：阶段 → 这个阶段的能力逐个勾选、勾上就按描述符给参数输入框（布尔是 SquishSwitch，数字 / 字符串是输入框，空着就是缺省值；每次调用时才定的参数如 run_id 不给）、箭头展开五栏；断点 → 「发布」「验收」两个快捷、一个确认事项输入框。检查结果在左下角：通过就说经过哪几个阶段，有问题一条一条列、并在节点上打红点（悬停看句子）；提醒琥珀色。窄屏配置改成右侧抽屉。

对齐：全部左对齐；数字右对齐成列。

## Components

shadcn（radix-nova 预设）作基础件；编辑台的画布是 React Flow（`@xyflow/react`，MIT，Dify 同款引擎），点阵、边、缩放按钮的颜色走 tokens。reactbits 只用在承担信息的地方，从 registry 捞来改装、走 tokens：Dock（地方栏：横改竖、块是真按钮、名字靠右提示、减少动效不放大）、ElectricBorder（正在跑的那一步）、CountUp（结果三个大数字）、SpotlightCard（工作流墙）、SpecularButton / StarBorder（发布、验收两颗键，全页只有它们有这种质感）、ShinyText（作业跑着时的状态字）、GlassSurface（输入框的壳，跟 `data-theme`）、FoldText（面板的名字像折纸展开，gsap 换成 motion）、TiltedCard（门口那张封面卡，换名字换图）、GlareHover（对话抽屉的入口按钮，改成真按钮）、GlassIcon（顶栏那枚标记，GlassIcons 只留一枚）、AnimatedList（工作区清单与对话清单逐条浮现）、SquishSwitch（页眉的深浅色开关：滑块拖得越快拉得越长，颜色走 tokens、做小；编辑台里布尔参数也用它）、GlideSelect（输入框上「模型」「思考」两枚下拉片：菜单从片的一角弹出、高亮在行间滑动，改装成挂在 body 上定位——玻璃壳 overflow-hidden 会切菜单——片上带前缀字、图标换 Phosphor、颜色走 tokens）。`components/Band` 是自家的：一张图 + 一层纱幕，顶栏（wash，糊成颜色）、抽屉与库（foot，脚下压纸色）都用它；`components/Scene` 也是自家的：铺满整块面板的画面，欢迎屏（side，左边压纸色给字站）、对话正文与脊柱那列（mist，压七成只剩氛围）用它。图标 Phosphor（regular；看板开着那颗用 fill），全站一套，`components.json` 的 `iconLibrary` 也是它，shadcn 再生成的件不会带回 lucide。
