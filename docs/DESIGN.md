...
给不写代码的研究者用的科研工作台。页面头号任务一句话：现在做到哪一步、在等谁、数字可不可信。世界观来自实验记录本和评分脚本——账本、基线、统计门、验证、发布与验收两处人的确认。视觉从这儿长，不从 SaaS 模板长。
...
| `--wait` 琥珀 | `#B8741A` | `#E2A751` | 只给「等你确认」：发布、验收、流里的断点 |
...
- **思源宋体（Noto Serif SC Variable）**：只给页面上的一句话标题（`.t-lede`：欢迎语、库的板块名）、脊柱上每一步的标题（`.t-step`）、Markdown 里的标题、顶栏名字。助理的回复按普通 Markdown 排，不拎结论（主人：别限得这么死，让它自己用 Markdown 发挥）。期刊味，和实验记录本对味。
