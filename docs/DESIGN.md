...
- **编辑台**（改库，[#100](https://github.com/zephyr4123/TJU-AI4Science/issues/100)）：一整张画布铺满（React Flow），底下压一层风景（晨光山脊线，mist 纱幕，与脊柱那列同一做法），点阵铺在风景上。不是表单：左上角一根**阶段梯**——七个研究阶段按顺序竖排成方块（大图标、宋体名），底下三种断点（琥珀）；梯子右边是**题头**——一块玻璃板（GlassSurface，与门口的输入框同款），宋体大标题、mono 名字、一行说明直接写在板上，没有框没有线，聚焦整块板亮一圈；右上角「库 N」弹层与圆角「保存」，名字与库里重了才出现「覆盖同名」的开关。造流助理不占一列：右下角一枚圆形玻璃入口（reactbits Magnet 磁吸 + ClickSpark），点开在角上长出一扇悬浮的对话窗（GlassSurface 壳，motion 从右下角缩放长出来；窗里就是对话那一列：抽屉、对话、输入框，右上角一枚关闭），再点收起——画布是主角，对话是辅助。梯子上的块拖进画布落在哪条边上就插在哪，点一下加到末尾。节点是一张卡：阶段卡（SpotlightCard 改装：22px 图标、宋体阶段名、点名的能力做靛色小片，没点名写「不限能力」，右下角一枚淡的宋体大序号——序号是文件里真实的第几项）、断点卡（琥珀底：图标 + 确认事项两行，发布盖章、验收签名）；边是折线带箭头，只表示顺序；一行放不下换行，坐标不进文件。选中一个节点右上角浮出配置：阶段 → 这个阶段的能力逐个勾选、勾上就按描述符给参数输入框（布尔是 SquishSwitch，数字 / 字符串是输入框，空着就是缺省值；每次调用时才定的参数如 run_id 不给）、箭头展开五栏；断点 → 「发布」「验收」两个快捷、一个确认事项输入框。检查结果在左下角：通过就说经过哪几个阶段，有问题一条一条列、并在节点上打红点（悬停看句子）；提醒琥珀色。窄屏配置改成右侧抽屉。
