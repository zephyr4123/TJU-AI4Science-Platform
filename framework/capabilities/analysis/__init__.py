"""写分析初稿：分析阶段里现有的一个能力——读一次或几次实验的账本、笔记、diff 与结果，
...
DESCRIPTOR = Capability(
    ...
    stage="分析",
    title="分析初稿",
    brief="按账本、笔记与结果写三节固定的分析初稿",
    does=(
        "起一个执行层会话，只给它点名的每次实验的账本全部行、实验笔记全文、"
    ...
    does_not=(
        "不核对数字对不对（那是验证阶段「数字核对」的事）、不画图、不写综述。"
        "它是初稿，不是分析这个阶段的全部：以后加进来的对比、作图、复盘都是这个阶段里另外的能力。"
        "执行层只许写 analysis.md，写了别的判失败。"
    ),
    brings=(
        "一次或几次跑过至少一轮的实验产出：scoring.yaml、ledger.tsv、"
