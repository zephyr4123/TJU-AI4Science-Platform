"""auto-research：实验阶段的那颗能力——读设计的产出开一次实验，然后一轮一轮改代码，统计门棘轮
（workflow.md §2）。
...
（`--resume`）、停了续命都是接着同一次产出干（`--continue experiment/<n>`），不另开目录——
那些是同一次实验的后续，不是新实验。
"""
...
DESCRIPTOR = Capability(
    ...
    does=(
        ...
        "work/ 硬回退到 best。账本 ledger.tsv 每轮一行（commit、指标、σ、耗时、"
        "花费、裁决），实验笔记 notebook.md 每轮一段，下一轮整本进提示。"
    ),
    does_not=(
        "不碰 harness/（目录 hash 每轮核对，越界改动判 readonly_violated 并回滚）、"
        "不改 scoring 快照里的指标与预算（续命除外）、不写分析、"
        "不判结果可不可信——它只负责让分支 tip 永远是目前最好的那一版。"
    ),
    brings=(
        "设计阶段跑过基线的那包产出（--from design/<n>）：harness/（封好的评分脚本）、"
        "code/（基线代码）、"
        "baseline/（基线成绩与 σ）、scoring.yaml（指标、方向、budget：max_iterations、patience、"
        "max_cost_usd、inner_k、统计门）、env/。接着跑带 --continue experiment/<n>；"
        "被杀在半路再带 --resume；已停的要续命带新的预算与 --reason。"
    ),
    leaves=(
        ...
        "best_commit、stop_reason）、ledger.tsv、notebook.md、"
        "iters/iter_N/（每轮的代码快照与 results.json）、journal.md（协调层的本子）。"
    ),
    stops=(
        "到了 budget.max_iterations，或连续 patience 轮没改进，或累计花费过了 max_cost_usd，"
        "或执行层连续几轮不可修复地失败——停止原因写进 checkpoint 与 stop.json。"
        "给了 --max-iters 就只跑这么多轮再退出（实验不算停，下次 --continue 接着跑）。"
        "开实验那一步就拒的情况：设计那包不合约、预检说没有改进空间、环境建不出来——"
        "前置检查失败就不开，不留半截。"
    ),
    params=(
        Param("max_iters", "int", None,
              "本次增量最多跑几轮；上限仍是 scoring 的 budget.max_iterations"),
        Param("resume", "bool", False,
              "上次被杀在半路：先做 checkpoint × 账本 × git 三方对账，收尸后接着跑", in_flow=False),
        ...
        Param("reason", "str", "", "续命的原因，记进 journal.md", in_flow=False),
    ),
    needs_executor=True,
    needs_compute=True,
    continuable=True,
...
def run(
    ...
    """统一入口。产出目录还没铺就开实验（读 --from 的设计那包）；铺过（--continue）就按参数续命
    ...
    if not opened:
        if resume or extending or reason:
            raise CapabilityFailed(
                "--resume 与续命参数只对已经开过的实验有意义：带 --continue experiment/<n>")
