...
判定是框架做的，执行层不参与判自己的结果（P-2）。分类结果连同一句修复提示喂给下一轮，
...
# 不是失败分类、判决仍是 discard，但分数与 best 完全相同时要单独提醒（gate.NO_EFFECT_NOTE）
NO_EFFECT_HINT = ("上一轮的分数与 best 完全相同：改动没有影响结果。先确认新代码路径真的被执行到"
                  "（比如一次只撒 1 个起点时换撒点方式等于没换），再决定改什么")
