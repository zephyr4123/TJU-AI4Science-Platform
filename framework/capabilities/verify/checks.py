"""验证能力的检查项：每项一个函数，输入是 run 目录里的产物，输出一份 `Check`。
...
整数与百分比不查——前者是计数，后者是相对变化，没有绝对来源（`contracts.analysis` 的边界）。
"""
...
from framework.contracts.analysis import Claim, parse_claims, prose_numbers
from framework.contracts.results import read_metrics
from framework.memory import ledger
from framework.run import artifacts, layout
...
def analysis_present(run_dir: Path) -> Check:
    doc = layout.analysis_doc(run_dir)
    if doc.is_file() and doc.read_text(encoding="utf-8").strip():
        return Check("analysis_present", True, [str(doc.relative_to(run_dir))])
    return Check("analysis_present", False, [f"{doc.relative_to(run_dir)} 不存在或为空"])


def numbers_traceable(run_dir: Path, text: str, tolerance: float) -> Check:
    """数据表每一行 (run, 指标, 值) 都能在那个 run 的 results.json 里找到。"""
    claims, problems = parse_claims(text)
    details = list(problems)
    sources = _load_sources(run_dir)
    for claim in claims:
        actual = sources.get(claim.run, {}).get(claim.metric)
        if actual is None:
            details.append(f"第 {claim.line_no} 行：{claim.run} 没有指标 {claim.metric} 的结果")
        elif not _close(claim.value, actual, tolerance):
            details.append(f"第 {claim.line_no} 行：{claim.run} 的 {claim.metric} 声称 "
...
def ledger_reconciled(run_dir: Path) -> Check:
    """账本 × git 对账：分析引用的账本本身得是真的。复用内环那把尺子。"""
    problems = ledger.reconcile(layout.ledger(run_dir), layout.work(run_dir))
    if problems:
        return Check("ledger_reconciled", False, problems)
    return Check("ledger_reconciled", True, ["账本每行 commit 与 git 对得上"])


def _load_sources(run_dir: Path) -> dict[str, dict[str, float]]:
    """run 名 → 指标 → 实际值；不合约的 results.json 不进来，回溯到它就是"没有结果"。"""
    sources: dict[str, dict[str, float]] = {}
    for name, path in artifacts.run_results(run_dir).items():
        metrics, _ = read_metrics(path)
        if metrics is not None:
            sources[name] = metrics
    return sources
