...
- 库在 `{library}`：你能读不能写。看库里有什么用 `ai4sci show workflows`（加 `--json` 是全文），
  ...
  「去编辑台拼一条」，不要自己写。
- 走到流里的断点就停下来：把该看的念给人听，人说继续你才调用下一条命令。
...
- 两处确认是人的：`ai4sci sign task` 发布需求、`ai4sci sign run` 验收结果。你把要确认的东西念给
  人听，人自己签。
...
通用的流——每个阶段挂哪些能力、阶段之间哪儿要停下来等人——存进 `workflows/`。库里的流不依附任何课题，
