"""作业跑完叫醒协调 agent：以「框架」的身份给那段对话发一轮（外层 #63）。

在 chat 层：它认识对话与后端，不认识能力。调用方是 cli 层的作业子进程——作业记录里有 chat_id
（起 agent 时经 `AI4SCI_CHAT_ID` 传给它按的按钮，`--detach` 记下来），跑完就来这里。
...
def message_for(job: Job) -> str:
    ...
    return (f"作业 {job.job_id}（`ai4sci {' '.join(job.argv)}`）{verdict}：\n{job.result}\n\n"
            ...
            "下一步打算怎么办；要按下一颗按钮就按，长的照旧 --detach。")


def wake(runs_root: Path, job: Job, *, retry_s: float = RETRY_S, max_wait_s: float = MAX_WAIT_S,
         sleep: Callable[[float], None] = time.sleep) -> str:
    """发一轮，返回一句状态：done / error: …（那一轮没走完）/ busy: …（等不到）/ failed: …。"""
    assert job.chat_id, "没有 chat_id 的作业不该来叫醒"
    try:
        conv = conversation.load_conversation(runs_root, job.chat_id)
