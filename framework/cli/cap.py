"""`ai4sci cap list | <name> <run_id>`：按名字跑一个能力的通用驱动（纲领 P-10、P-12）。

在 cli 层。每个能力的子命令是从它的描述符**生成**的：`--backend` 只在 needs_executor 时有，
...
def cmd_cap(args: argparse.Namespace) -> int:
    ...
    setup_logging()
    try:
        line = args.module.run(run_dir, ports, **params)
    except (CapabilityFailed, TaskInvalid) as exc:
