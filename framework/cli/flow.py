...
在 cli 层。流分两层：库（`workflows/`，造流助理改）→ 实例（工作区 `flows/`，研究助理按这份需求
改参数、增删阶段与断点）。取流就是把库里那份复制成实例，过一遍同样的检查再落盘；之后
研究助理直接改 `flows/<name>.yaml`，`show flows` 校验，每颗能力 `--flow <name>` 照它跑
（只有一条流时可省）。
...
def cmd_take(args: argparse.Namespace) -> int:
    ...
    if not source.is_file():
        ...
        print(f"库里没有叫 {args.name!r} 的流（有：{available}）", file=sys.stderr)
        return EXIT_USAGE
    raw = yaml.safe_load(source.read_text(encoding="utf-8"))
    name = args.as_name or args.name
    if isinstance(raw, dict):
        raw["name"] = name  # 实例可以换个名字：同一条库里的流按两种参数各取一份
...
def add_parser(groups: argparse._SubParsersAction) -> None:
    flow = groups.add_parser("flow", help="流实例：把库里的流取到当前工作区")
    ...
    taking.add_argument("name", help="库里的流名（ai4sci show workflows）")
