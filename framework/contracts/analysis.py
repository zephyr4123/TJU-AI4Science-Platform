...
执行层必须把它引用的每个指标值列进 `## 数据` 表（run、指标、值三列，值从 results.json
原样抄），正文里带小数点或指数的数只许出现表里有的值。分析能力在交产物前校验形状，
...
DATA_HEADING = "## 数据"
TABLE_COLUMNS = ("run", "指标", "值")
RUN_ID_RE = re.compile(r"^run_\d+$")
# 带小数点或指数的数；前后不能贴着字母、数字、点（避免切开 run_0.5、v1.2.3 这类标识），
# 后面紧跟 % 的是百分比，不算
...
class Claim:
    """数据表的一行：某个 run 的某个指标是这个值。line_no 是在 analysis.md 里的行号，报问题用。"""
...
def parse_claims(text: str) -> tuple[list[Claim], list[str]]:
    ...
    for line_no, line in _section_lines(text, DATA_HEADING):
        ...
        if len(cells) < 3:
            ...
            continue
        run, metric, raw = cells[:3]
        if not RUN_ID_RE.match(run):
            problems.append(f"第 {line_no} 行：run 列要形如 run_N，得到 {run!r}")
            continue
        try:
            value = float(raw)
        except ValueError:
            ...
            continue
        claims.append(Claim(run=run, metric=metric, value=value, line_no=line_no))
    return claims, problems
