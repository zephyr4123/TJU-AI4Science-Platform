...
三层：**阶段**（七间房，固定）→ **能力**（一间里的一件活，对 agent 就是一个 tool：一条 `ai4sci cap`
命令）→ **实现**（这件活怎么干：一段代码、一个 skill、领域包里的东西）。描述符说的是中间那层：
它属于哪一间、干什么、不干什么、要带什么进来、留下什么、什么时候停。五栏都是给人读的——研究者、
协调 agent、工程师读同一份——讲机制、带通用的专业术语，不讲路径表：能力之间不接管子，下一颗进门
自己看盘上有什么（P-18）。
...
# task：动任务包（说清课题、写裁判跑基线、开一次实验），入口 run(workspace, ports, ...)；
# run：动一个 run，入口 run(run_dir, ports, ...)；project 还没有实例
LEVELS = ("task", "run", "project")
# 做科研的七间房（纲领 workflow §1，P-18）。每颗能力属于且只属于一间；一间可以有几颗能力，也可以
# 暂时空着（清单里照样列出来，让人看见缺什么）。房间不定先后、没有代码、没有运行时——顺序归流与
