...
写进 manifest（`metrics[].attainable`），这三四个数的比较就是零模型的算术，不用人守着。

两条硬判定，判中就停、不开跑：
- 门是 0：σ=0 且没写 min_delta，任何噪声都算改进，等于没门。
- 基线到尽头的距离 ≤ 门：改进再大也过不了门，这道题在这个门下无解，得改题、松门或换基线。

门高的算式只在这里定义一次：内环的统计门（capabilities/experiment/gate.py）调的也是它。
预检说"还有 1.7 个门的空间"，内环用的就是同一把尺子。

在 contracts 层：读 manifest 与 run_0，不认识 run 目录。
"""
...
# manifest 不写时的缺省，与 run/context.py 的 DEFAULT_MIN_DELTA 同值：那边是内环的读取点，
...
def assess(task_dir: Path) -> Headroom:
    """读 manifest 与 run_0，算出上面那几个数。要求包已经有合约的 run_0（validate 管形状）。"""
    task_dir = Path(task_dir)
    manifest = yaml.safe_load((task_dir / packs.MANIFEST_NAME).read_text(encoding="utf-8"))
    metric = _primary_metric(manifest)
...
def _primary_metric(manifest: dict[str, Any]) -> dict[str, Any]:
...
def _read_json(path: Path) -> Any:
    if not path.is_file():
        raise FileNotFoundError(f"预检要读 {path}，文件不在：基线没跑完或 run_0 不合约")
