"""产出：一个阶段目录下的一次产出 `<stage>/<n>/`，框架只认它里面的两份文件（纲领 P-19）。
    ...
    meta.yaml     谁产的、读了谁、在哪条流第几项下产的、按哪版需求、成没成——框架写，能力不碰
    signed.json   人的签字：流里这一项后面有断点才需要；签的是这个目录里全部文件的 hash

其余文件全归产它的那颗能力，框架不看、不定、不校验。
...
在契约层：只读写这两份文件与算目录 hash，不认识流、能力、对话。
"""
