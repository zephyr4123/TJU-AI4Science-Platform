"""七个研究阶段：名字（给人看、流文件里写的）与英文 slug（目录名、id、URL 里用的），一张表只此一处
（纲领 P-18、P-19）。

阶段不定先后、没有代码、没有运行时——顺序归流与协调层（P-10），这里只回答"这个阶段叫什么、目录叫什么"。
表的顺序是页面与 `show caps` 列清单的顺序，也是磁盘上七个目录的固定序（工作区里只建流走过的）。
