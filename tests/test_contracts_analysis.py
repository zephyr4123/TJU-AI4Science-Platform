...
from framework.contracts.analysis import parse_claims, prose_numbers, validate_analysis

GOOD = """# t
## 结论
best 是 run_3 的 0.001，比 run_0 的 0.03 降了 96.7%，共 3 轮；`code/x.py:0.5` 是文件。
## 数据
| run | 指标 | 值 |
|---|---|---|
| run_0 | val_mse | 0.03 |
...
def test_validate_ok_and_claims_parse():
    ...
    assert problems == []
    assert [(c.run, c.metric, c.value) for c in claims] == [
        ("run_0", "val_mse", 0.03), ("run_3", "val_mse", 0.001)]
    assert [c.line_no for c in claims] == [7, 8]
...
def test_wrong_header_stops_parsing_with_a_problem():
    ...
    assert claims == []
    assert problems == ["第 2 行：数据表表头必须是 | run | 指标 | 值 |"]
...
def test_scientific_and_signed_numbers_and_identifiers():
    text = "## 结论\n-1.5e-3 与 2E5 算，run_0.5 与 v1.2.3 不算，12.5 % 也不算。\n"
