...
（test_loop.py）只能证明"整条链连得上"，证明不了优先级的顺序对不对。
...
from framework import failures, gitwork, ledger, loop
from tests.fixtures import packs_factory as pf
from tests.fixtures.scripted_backend import ScriptedRunner
from tests.test_loop import make_loop_pack, train_for_mse
...
def test_read_results_takes_nan_through_so_it_can_be_classified(tmp_path):
    ...
    metric, problems = failures.read_results(path, "val_mse")
...
def test_read_results_missing_file_and_broken_json(tmp_path):
    metric, problems = failures.read_results(tmp_path / "没有.json", "val_mse")
    ...
    metric, problems = failures.read_results(broken, "val_mse")
...
def test_harness_touching_data_is_readonly_violated_and_rolls_back(tmp_path):
    ...
    run_dir = loop.new_run(pack.task_dir, tmp_path / "runs", "r-tampered",
                           domains_root=pack.domains_root)
    work = run_dir / "work"
    before = (work / "data" / "val.json").read_text(encoding="utf-8")

    loop.run_loop(run_dir, ScriptedRunner([train_for_mse(0.001)]), LocalCompute(), max_iters=1)
