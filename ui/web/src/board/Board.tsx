...
export function Board({ workspace, doc, caps, opened, onOpen, onOpenFiles }: {
  workspace: string; doc: Resource<WorkspaceDetail>; caps: Resource<Capability[]>; opened: string | null
  ...
  // 每一列底下写这一步的能力（点名的，或这个阶段能用的）：名直接显示、一行 hover（P-21）
  const chip = (c: Capability) => ({ title: c.title, brief: c.brief })
  const capsOf = (stage: string, named: string[]) => (named.length
    ? { caps: named.map((name) => { const c = catalog.find((x) => x.name === name); return c ? chip(c) : { title: name, brief: '' } }), named: true }
    : { caps: catalog.filter((c) => c.stage === stage).map(chip), named: false })
