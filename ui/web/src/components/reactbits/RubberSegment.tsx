...
export default function RubberSegment({
  ...
  return (
      ...
      style={
        {
          '--rs-track': 'var(--muted)',
          '--rs-thumb': 'var(--card)',
          '--rs-ink': 'var(--muted-foreground)',
          '--rs-ink-active': 'var(--foreground)',
          '--rs-radius': `${radius}px`,
          '--rs-inset': `${inset}px`,
    ...
    >
      ...
      <motion.div
        className="pointer-events-none absolute inset-[var(--rs-inset)] grid grid-flow-col auto-cols-auto group-data-[equal]:auto-cols-[minmax(0,1fr)] [background:var(--rs-thumb)] [color:var(--rs-ink-active)] shadow-sm"
        aria-hidden="true"
        style={{ clipPath }}
      >
        {list.map(item => (
