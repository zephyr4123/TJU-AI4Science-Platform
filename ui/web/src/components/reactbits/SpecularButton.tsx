// 对话入口那颗键，从 reactbits 的 SpecularButton 捞来改装（MIT，https://reactbits.dev/components/specular-button）：
// 玻璃底、一圈随鼠标走的高光（WebGL，ogl）。改动：颜色走 tokens（父组件用 useToken 读值传进来）、减少动效时不扫光、
// 字号与内边距按站内的尺寸；别的原样。全站只有工作区与编辑台右下角的「打开对话」用它。
...
const SpecularButton = ({
  ...
  // 原件在渲染期直接赋 ref；改成 effect 里同步，动画循环读到的仍是最新值
  useEffect(() => {
