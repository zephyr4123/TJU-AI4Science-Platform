...
const CLI_SENTENCE: [RegExp, string][] = [
  ...
  [/ai4sci task validate/, '校验了任务包'],
  [/ai4sci task publish/, '按了发布键（这颗键应该由人按）'],
  [/ai4sci task env build/, '给任务建了运行环境'],
  [/ai4sci cap design/, '接了任务：写裁判脚本和基线草稿'],
  [/ai4sci cap baseline/, '跑了基线'],
  [/ai4sci cap analysis/, '写了分析'],
  [/ai4sci cap verify/, '验证了分析里的数字'],
  [/ai4sci cap list/, '看了一眼有哪些按钮'],
  [/ai4sci run new/, '建了一次实验'],
  [/ai4sci run extend/, '给实验续了预算'],
  [/ai4sci run accept/, '按了验收键（这颗键应该由人按）'],
  [/ai4sci loop (run|resume)/, '跑了几轮实验'],
  [/ai4sci status/, '看了实验状态'],
  [/ai4sci flow check/, '检查了流程通不通'],
