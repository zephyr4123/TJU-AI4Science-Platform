...
// 工作区的文件夹名与流程的文件名同一条规矩（P-21：机器的名字由标题生成，不显示、不让填），只差前缀。
...
export function suggestId(title: string, taken: Iterable<string>, today: Date, prefix = 'ws'): string {
  ...
  if (base === '') base = `${prefix}-${two(today.getMonth() + 1)}${two(today.getDate())}`
  else if (!/^[a-z]/.test(base)) base = `${prefix}-${base}`
