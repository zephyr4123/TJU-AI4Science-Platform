// 地方栏：先选世界，再选世界里的东西（外层 #79 #80）。顶上两块是双向开关——工作区 / 编辑台，两个平行的世界（纲领 P-15 P-16）：
// 工作区有很多个、各有自己的对话与脊柱；编辑台是全局一个库、有自己的对话。开关下面那段只属于当前世界：工作区世界里是一块块封面
        ...
        <AnimatePresence initial={false}>
          {world === 'workspace' && (
            <motion.div key="workspaces" initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }}
                        ...
                        className="flex min-h-0 flex-1 flex-col items-center gap-2.5 self-stretch overflow-y-auto py-3 [scrollbar-width:none]">
