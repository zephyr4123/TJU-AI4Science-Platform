...
// 断点：确认事项，「发布」「验收」是出厂的两个。五列说明折在展开层里，不占面板。
  ...
  return (
    <div>
      ...
      <div className="mt-3 flex gap-2">
        {(['发布', '验收'] as const).map((note) => (
          <Button key={note} size="sm" variant={item.note === note ? 'default' : 'outline'} onClick={() => onChange({ ...item, note })}>{note}</Button>
        ))}
      </div>
